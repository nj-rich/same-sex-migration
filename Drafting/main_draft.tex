...
\end{table}
%\FloatBarrier
Here, I run the same regressions as above, but only look at men. Tables \ref{tab: male_expost_model} and \ref{tab: male_exante_model} report regression results for the pull factor and push factor models respectively. Results for a women-only specification can be found in the appendix (Tables \ref{tab: female_expost_model} and \ref{tab: female_exante_model} and figures \ref{fig: women_post_diffs} and \ref{fig: women_ante_diffs}).
%\FloatBarrier
\begin{figure}[htbp]
    \centering
...
\begin{table}[htbp]
    ...
    \input{outputs/regressions/ffed_expost_model}
\end{table}
\begin{table}[htbp] %finagling to get formatting right
    \centering
