...
count_in_new_legal <- summary_stats %>%
  group_by(year) %>%
  summarize(prop_in_new_legal = survey_mean((1-expost_old_legal), na.rm = T)) %>%
  select(-prop_in_new_legal_se)

gc()
#create joint table
joint_overall_table <- count_male_female %>%
  left_join(count_samesex, by = "year") %>%
  left_join(count_in_new_legal, by = "year") %>%
  mutate(across(-year, ~ . * 100)) %>%
  setNames(c("Year", "% Female", "% Same-Sex", "% in States Without State-Level Marriage Equality")) %>%
...
joint_var_table_1 <- prop_mig_samesex %>%
  ...
  kable(caption = "Summary Statistics for Main Variables (1)", format = "latex", booktabs = T, digits = 3) %>%
...
joint_var_table_2 <- mean_age_samesex %>%
  ...
  kable(caption = "Summary Statistics for Main Variables (2)", format = "latex", booktabs = T, digits = 3) %>%
...
regions_1 <- prop_region_samesex %>%
  ...
  mutate(across(-year, ~ . * 100)) %>%
  setNames(c("Year", "Same-Sex", "Opposite-Sex", "Same-Sex", "Opposite-Sex")) %>% #needs work
  kable(caption = "Summary Statistics by Region (1)", format = "latex", booktabs = T, digits = 3) %>%
...
post_migrant_diffs_chart <- post_migrant_trendsdiffs_table %>%
  ...
  ggplot() +
    ...
    labs(title = "Pull Factor Triple Regression Visualized",
         x = "Year",
         y = "Migration Difference by State Type",
         linetype = "Subgroup") + #maybe move title to plot itself
...
make_flowdiffs_chart <- function(df, sel_title) {
  ...
  ggplot() +
    ...
    labs(title = sel_title,
         x = "Year",
         y = "Migration Difference by State Type", #ambiguous for purposes of from v. to
         linetype = "Subgroup") + 
...
stay_post_diffs_chart <- stay_post_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_flowdiffs_chart(sel_title = "Pull Factor Diffs From Stay")
...
fed_ante_diffs_chart <- fed_ante_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_flowdiffs_chart(sel_title = "Push Factor Diffs To Not/Fed Legalized") 
...
local_ante_diffs_chart <- local_ante_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_flowdiffs_chart(sel_title = "Push Factor Diffs To Locally Legalized")
...
stay_ante_diffs_chart <- stay_ante_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_flowdiffs_chart(sel_title = "Push Factor Diffs To Stay")
...
sex_post_diffs_chart <- post_migrant_trends_df %>%
  ...
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  mutate(sex_label = if_else(sex == 1, "male", "female")) %>%
  make_diffs_chart(sel_title = "Pull Factor Diffs by Gender", group = sex_label, facet = "sex_label", facet_titles = c("male" = "Men", "female" = "Women")) #use male/female?
sex_post_diffs_chart
gc()
#Ante
...
sex_ante_diffs_chart <- post_migrant_trends_df %>%
  ...
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  mutate(sex_label = if_else(sex == 1, "male", "female")) %>%
  make_diffs_chart(sel_title = "Push Factor Diffs by Gender", group = sex_label, facet = "sex_label", facet_titles = c("male" = "Men", "female" = "Women")) #use male/female?
sex_ante_diffs_chart
...
age_post_diffs_chart <- post_migrant_trends_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_diffs_chart(sel_title = "Pull Factor Diffs by Age", group = age_groups, facet = "age_groups", c("<23" = "<23", "23-32" = "23-32", "33-62" = "33-62", ">62" = ">62"))
...
age_ante_diffs_chart <- ante_migrant_trends_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_diffs_chart(sel_title = "Push Factor Diffs by Age", group = age_groups, facet = "age_groups", c("<23" = "<23", "23-32" = "23-32", "33-62" = "33-62", ">62" = ">62"))
...
educ_post_diffs_chart <- post_migrant_trends_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_diffs_chart(sel_title = "Pull Factor Diffs by Education", group = college, facet = "college", facet_titles = c("college" = ">=4 Years College", "notcollege" = "<4 Years College"))
...
educ_ante_diffs_chart <- post_migrant_trends_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_diffs_chart(sel_title = "Push Factor Diffs by Education", group = college, facet = "college", facet_titles = c("college" = ">=4 Years College", "notcollege" = "<4 Years College"))
...
child_post_diffs_chart <- post_migrant_trends_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_diffs_chart(sel_title = "Pull Factor Diffs by Presence of Children", group = child, facet = "child", facet_titles = c("yes" = "Children Present", "no" = "Children not Present"))
...
child_ante_diffs_chart <- post_migrant_trends_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_diffs_chart(sel_title = "Push Factor Diffs by Presence of Children", group = child, facet = "child", facet_titles = c("yes" = "Children Present", "no" = "Children not Present"))
...
inc_post_diffs_chart <- post_migrant_trends_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_diffs_chart(sel_title = "Pull Factor Diffs by Income", group = income_group, facet = "income_group", facet_titles = c("above" = "Above Mean", "below" = "Below Mean"))
...
inc_ante_diffs_chart <- post_migrant_trends_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_diffs_chart(sel_title = "Push Factor Diffs by Income", group = income_group, facet = "income_group", facet_titles = c("above" = "Above Mean", "below" = "Below Mean"))
...
region_post_diffs_chart <- post_migrant_trends_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_diffs_chart(sel_title = "Pull Factor Diffs by Region", group = expost_region, facet = "expost_region", facet_titles = c("Midwest" = "Midwest", "West" = "West", "South" = "South", "Northeast" = "Northeast"))
...
region_ante_diffs_chart <- post_migrant_trends_df %>%
  ...
  mutate(samesex_diff = samesex_oldlegal - samesex_newlegal) %>%
  mutate(oppositesex_diff = oppositesex_oldlegal - oppositesex_newlegal) %>%
  make_diffs_chart(sel_title = "Push Factor Diffs by Region", group = exante_region, facet = "exante_region", facet_titles = c("Midwest" = "Midwest", "West" = "West", "South" = "South", "Northeast" = "Northeast"))
region_ante_diffs_chart
...
ex_post_diffs_chart <- post_ex %>%
  make_ex_diffs_chart(sel_title = "Predicted Pull Factor Differences")

ex_ante_diffs_chart <- ante_ex %>%
  make_ex_diffs_chart(sel_title = "Predicted Push Factor Differences")
...
pop_support_export <-pop_support %>%
  kable(caption = "Support for Gay Men and Lesbian Women by State in 2016", format = "latex", booktabs = F, digits = 3, longtable = T)
