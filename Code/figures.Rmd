...
Summary stats - for regression storytelling (note: might need to deprecate- doesn't really match triple diff charts, watch utilty; might have to cut)
```{r, include = F}
...
Make trends graphs (need to make accompanying tables for this, see replace above chunk)
```{r, include = F}
gc()
post_migrant_trends_df <- summary_stats %>%
  mutate(subgroup = case_when(
    ...
    in_samesex == 0 & expost_old_legal == 0 ~ "oppositesex_newlegal" #hopefully ok no final statement and all cases covered
  )) %>%
  group_by(year, subgroup) %>%
  summarize(frac_migrate = survey_mean(migrant, na.rm = T)) %>%
  select(-frac_migrate_se) #%>%
  #pivot_wider(names_from = subgroup, values_from = frac_migrate) #for presentation split back out

gc()
post_migrant_trends <- post_migrant_trends_df %>%
  mutate(year = factor(year)) %>%
  ggplot() +
    geom_point(aes(x = year, y = frac_migrate, color = subgroup)) +
  labs(title = "Ex-Post Triple Regression Visualized",
         y = "Fraction Migrated",
         color = "Subgroup") +
  scale_color_manual(values = c("samesex_oldlegal" = "blue", "opppositesex_oldlegal" = "red", "samesex_newlegal" = "yellow", "oppositesex_newlegal" = "green"),
...
#watch lotta work still needed here
post_migrant_trends

#ante

#hmmm... think: group_bys or mutates? YUP NEED TO SEPARATE GROUPS
#oop don't split out and do color by group
#so pretty:) oopsy maybe doesn't support me oh well
#watch stock v flow variables:)
#forced choice: better looking x axis v geom_line (should be able to resolve at some point)
#oh god this is so ugly but eh... keep thinking
#watch colors
```
