...
(Deprecated) stats - stock migrants/general pops. in certain state categories
```{r, include = F}
...
pop_support <- clean_df %>% #don't need survey object here
  ...
  arrange(support) %>%
  rename("State" = expost_state, "Popular Support" = "support") %>%
  select(State, "Popular Support") 

pop_support_export <-pop_support %>%
  kable(caption = "Support for Gay Men and Lesbian Women by State in 2016", format = "latex", booktabs = T, digits = 3, longtable = T)
