...
Data cleaning- technical variable reclassification/filtering (mostly resolving missing occurences)
```{r, include = F}
gc()
...
#oop hope this doesn't crash
```

Data cleaning- model variable reclassification/filtering
```{r, include = F}
gc()
#tentatively nothing for now, see controls and 2011 thoughts
```

Base summary stats (AGAIN HAVEN'T RESTRICTED SAMPLE YET, NO SE, OI GO BACK AND CHECK LIT REVIEW)- functionalize?
```{r, include = F}
gc()
...
```{r, include = F}
gc()
#desired (ish) model - still missing stuff- way too big for computer to handle:
# model <- feols(migrant ~ in_samesex * old_legal * post_2015, + i(sex) + age + i(race) + i(educ) + i(occ) + i(ind) + i(bpl) + inctot| in_samesex + year + state_name + in_samesex^year + in_samesex^state_name + year^state_name, weights = perwt, data = stata_analysis)

#what computer can handle
model <- feols(migrant ~ in_samesex * old_legal * post_2015 + i(sex), weights = ~perwt, data = stata_analysis) #rip cluster, SE, how incl in_samesex, oh my goodness regression way too big, think through fixed effects and clustering where put relative to regression
summary(model)
...
gc()
set.seed(123)
data_sample <- slice_sample(stata_analysis, n = 10000)
