...
#THINK: cases where people don't have a partner, check filters
...
df_samesex <- df_samesex %>%
  mutate(date_legalization = case_when(
    ...
    date_legalization == "PR" ~ "2015", #from googling - DROP
    date_legalization == "OMI" ~ "2015", #from guessing - DROP
    TRUE ~ date_legalization 
  ))
...
df_samesex_mm <- df_samesex %>%
  filter(in_samesex == 1) %>% #CHECK THIS CUTS STUFF OUT -watch double v single &
  ...
  filter(MIGPLAC1 > 0 & MIGPLAC1 < 57) #get interstate migrants only #FIX THIS
...
Make pct same-sex by state and year var (see weighting/exclusions) - UPDATED - still getting opposite results; make sure population definition makes sense(see MM page 5 under the equation) (TURN INTO FINCTION)
```{r, include = F}
gc() 
df <- df_samesex_mm
...
samesex_staterate <- samesex_staterate %>%
  ...
  rename(staterate_year = value) #check if this works in pipeline later
...
```{r, include = F}
gc()
write.csv(mm_gen, "C:\\Users\\njrich\\Desktop\\same-sex-migration\\mm_gen.csv")
write_dta(mm_gen, "C:\\Users\\njrich\\Desktop\\same-sex-migration\\mm_gen.dta") #fill in rest by gender
